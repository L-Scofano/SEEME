...
from EgoHMR.models.egohmr.modulated_gcn.modulated_gcn import ModulatedGCN
from EgoHMR.models.egohmr.losses import Keypoint3DLoss, Keypoint2DLoss, ParameterLoss
from EgoHMR.utils.geometry import aa_to_rotmat, perspective_projection, rot6d_to_rotmat
from EgoHMR.utils.konia_transform import rotation_matrix_to_angle_axis
from EgoHMR.utils.other_utils import SMPL_EDGES
'''
...
class EgoHMR(nn.Module):
    def __init__(self, cfg, device=None,
        ...
        adj_mx = adj_mx#.to(self.scene_enc.device)
        ...
        self.smpl = smplx.create('./EgoHMR/data/smpl', model_type='smpl', gender='neutral')#.to(self.device)
        self.smpl_male = smplx.create('./EgoHMR/data/smpl', model_type='smpl', gender='male')#.to(self.device)
        self.smpl_female = smplx.create('./EgoHMR/data/smpl', model_type='smpl', gender='female')#.to(self.device)
    ...
    def pred_betas(self, conditioning_feats, body_pose=False):
        return self.beta_layer(conditioning_feats, body_pose)
    
    def encode_transl(self, transl):
        return self.transl_enc(transl)
    ...
    def mask_cond(self, cond, only_mask_img_cond=False, force_mask=False):
        ...
        if force_mask:
            ...
            mask = torch.bernoulli(torch.ones(bs) * self.cond_mask_prob).view(bs, 1)  # 1-> use null_cond, 0-> use real cond  [bs, 1]
            if only_mask_img_cond:
...
class FCHeadBeta(nn.Module):
    def __init__(self, in_dim=None, condition_on_pose=False, pose_dim=144):
        ...
        mean_params = np.load('./EgoHMR/data/smpl_mean_params.npz')
