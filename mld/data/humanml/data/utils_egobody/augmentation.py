...
def rot_aa(aa: np.array, rot: float, motion_length: int) -> np.array:
    """
    ...
    R = np.array([[np.cos(np.deg2rad(-rot)), -np.sin(np.deg2rad(-rot)), 0],
                  ...
                  [0, 0, 1]])
    # find the rotation of the body in camera frame, consider motion length
    per_rgd = []
    for i in range(motion_length):
        per_rdg, _ = cv2.Rodrigues(aa[i])
        resrot, _ = cv2.Rodrigues(np.dot(R,per_rdg))
        res = (resrot.T)[0]
        per_rgd.append(res)

    per_rdg = np.array(per_rgd)
    
    #per_rdg, _ = cv2.Rodrigues(aa) 

    # # consider motion length
...
def smpl_param_processing(smpl_params: Dict, has_smpl_params: Dict, rot: float, 
                          do_flip: bool, motion_length: int) -> Tuple[Dict, Dict]:
    """
    ...
    smpl_params['global_orient'] = rot_aa(smpl_params['global_orient'], rot, motion_length)
    return smpl_params, has_smpl_params
...
def get_example(img_path: str, center_x: float, center_y: float,
    ...
    if img_path != None:
        ...
        keypoints_2d_crop = None
        keypoints_2d_vis_mask = None
    ...
    if img_path != None:
        ...
        keypoints_3d_crop_noauge = None
        cam_t_crop = None
        cam_t_full = None

    if scene_pcd_verts is not None:
    ...
    if img_path != None:
        ...
        keypoints_3d_crop_auge = None
        keypoints_3d_full_auge = None

    if scene_pcd_verts is not None:
        scene_pcd_verts_full_auge = scene_pcd_verts_crop_auge - cam_t_crop + cam_t_full
    else:
        scene_pcd_verts_full_auge = None
