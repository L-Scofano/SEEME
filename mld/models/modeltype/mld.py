...
from mld.utils.geometry2 import (
    aa_to_quat,
    aa_to_rotmat,
...
class MLD(BaseModel):
    ...
    def __init__(self, cfg, datamodule, **kwargs):
        ...
        if self.data_type == "angle":
            ...
            self.smpl_model = smplx.SMPL(
                model_path=smpl_path, batch_size=cfg.TRAIN.BATCH_SIZE, gender="neutral"
            )
            # create_transl=False)
            ...
            self.smpl_model = smplx.create(
                "./datasets/data/smpl/", model_type="smpl", gender="neutral"
            ).double()
        ...
        if "scene" in self.condition or "image" in self.condition:
            use_proscene = True
            if use_proscene:
                self.proscene = ProHMRScene(
                    cfg=model_cfg,
                    with_focal_length=True,
                    with_bbox_info=True,
                    with_cam_center=True,
                    scene_feat_dim=512,
                    scene_cano=False,
                )
                weights = torch.load(
                    "./EgoHMR/checkpoints/checkpoints_egohmr/53618/best_model.pt",
                    map_location=lambda storage, loc: storage,
                )
                weights_copy = {}
                weights_copy["state_dict"] = {
                    k: v
                ...
                self.proscene2 = EgoHMR(
                    ...
                    scene_cano=True,
                    weight_loss_v2v=0.5,
                    weight_loss_keypoints_3d=0.05,
                    weight_loss_keypoints_3d_full=0.02,
                    weight_loss_keypoints_2d_full=0.01,
                    weight_loss_betas=0.0005,
                    weight_loss_body_pose=0.001,
                    weight_loss_global_orient=0.001,
                    weight_loss_pose_6d_ortho=0.1,
                    cond_mask_prob=0.01,
                    only_mask_img_cond=True,
                    weight_coap_penetration=0.0002,
                    start_coap_epoch=3,
                    pelvis_vis_loosen=False,
                )

                weights = torch.load(
                    "./EgoHMR/checkpoints/checkpoints_egohmr/91453/best_model_mpjpe_vis.pt",
                    map_location=lambda storage, loc: storage,
                )
                weights_copy = {}
                weights_copy["state_dict"] = {
    ...
    def _diffusion_reverse(self, encoder_hidden_states, lengths=None):
        # init latents
        bsz = encoder_hidden_states.shape[
            0
        ]  # * if classifier free then bs*2 and then it gets divided by 2
        ...
        for i, t in enumerate(timesteps):
            ...
            )  # * if classifier free then bs*2 or else it is bs
            lengths_reverse = (
    ...
    def train_vae_forward(self, batch):
        ...
        if self.vae_type in ["mld", "vposert", "actor"]:
            if self.estimate == "wearer":
                if self.predict_transl:
                    ...
                    motion_z, dist_m = self.vae.encode(
                        feats_ref[:, :, 0, :], cond_image_emb, lengths
                    )  # bs, 256
                ...
                if self.predict_transl:
                    ...
                    motion_z, dist_m = self.vae.encode(
                        feats_ref[:, :, 1, :], cond_image_emb, lengths
                    )  # bs, 256
        ...
        if self.data_type == "rot6d":
            ...
            feats_rst = rot6d_to_rotmat(feats_rst.reshape(-1, 6)).reshape(
                b, le, 24, 3, 3
            )
            # feats_rst = rotation_matrix_to_angle_axis(feats_rst).reshape(b, le, -1)

            feats_ref_rotmat = feats_ref
            feats_ref = rot6d_to_rotmat(feats_ref.reshape(-1, 6)).reshape(
                b, le, 24, 3, 3
            )
            # feats_ref = rotation_matrix_to_angle_axis(feats_ref).reshape(b, le, -1)

            smpl_ref = self.smpl_model(
                body_pose=feats_ref[:, :, 1:].reshape(-1, 23, 3, 3),
                global_orient=feats_ref[:, :, 0:1].reshape(-1, 1, 3, 3),
                pose2rot=False,
                return_full_pose=True,
                create_transl=False,
                create_beta=False,
            )
            smpl_rst = self.smpl_model(
                body_pose=feats_rst[:, :, 1:].reshape(-1, 23, 3, 3),
                global_orient=feats_rst[:, :, 0:1].reshape(-1, 1, 3, 3),
                pose2rot=False,
                return_full_pose=True,
                create_transl=False,
                create_beta=False,
            ...
            if self.name_dataset == "egobody":
                ...
                transl_ref = (
                    feats_ref[:, :, -3:].reshape(-1, 3).float()
                    if self.predict_transl
                    else transl[:, index_ref, :min_len, :].reshape(-1, 3).float()
                )
                joint_ref = self.smpl_model(
                    betas=betas_ref,
                    body_pose=body_pose_ref,
                    global_orient=global_pose_ref,
                    transl=transl_ref,
                    pose2rot=True,
                )
                joints_ref = joint_ref.joints.reshape(-1, feats_ref.shape[1], 45, 3)[
                    :, :, :24
                ]
                ...
                feats_rst = self.renorm(feats_rst)
                body_pose_rst = (
                    feats_rst[:, :min_len, 3:72].reshape(-1, 23 * 3).float()
                )  #! COMMENT
                ...
                transl_rst = (
                    feats_rst[:, :min_len, 72:].reshape(-1, 3).float()
                    if self.predict_transl
                    else transl[:, index_ref, :min_len, :].reshape(-1, 3).float()
                )
                joints_rst = self.smpl_model(
                    betas=betas_rst,
                    body_pose=body_pose_rst,
                    global_orient=global_pose_rst,
                    transl=transl_rst,
                    pose2rot=True,
                )
                joints_rst = joints_rst.joints.reshape(-1, feats_rst.shape[1], 45, 3)[
                    :, :, :24
                ]
                ...
                global_pose_ref = feats_ref[:, :min_len, :3].reshape(-1, 3).float()
                # transl_ref = feats_ref[:, :min_len, 69+10+3:].view(-1, 3)
                body_pose_ref = torch.cat(
                    [
                        body_pose_ref,
                        torch.zeros(body_pose_ref.shape[0], 6).to(body_pose_ref.device),
                    ],
                    dim=-1,
                )
                transl_ref = (
                ...
                feats_rst = self.renorm(feats_rst)
                body_pose_rst = (
                    feats_rst[:, :min_len, 3:66].reshape(-1, 21 * 3).float()
                )  #! COMMENT
                ...
                # transl_rst = feats_rst[:, :min_len, 69+10+3:].view(-1, 3)
                transl_rst = (
                    feats_rst[:, :min_len, -3:].reshape(-1, 3).float()
                    if self.predict_transl
                    else transl[:, index_ref, :min_len, :].reshape(-1, 3).float()
                )
                body_pose_rst = torch.cat(
                    [
                        body_pose_rst,
                        torch.zeros(body_pose_rst.shape[0], 6).to(body_pose_rst.device),
                    ],
                    dim=-1,
                )
                joints_rst = self.smpl_model(
                    ...
                    transl=transl_rst,
                    pose2rot=True,
                )  #! change orientation from ref to rst
                joints_rst = joints_rst.joints.reshape(-1, feats_rst.shape[1], 45, 3)[
                    :, :, :21
                ]
        ...
        rs_set = {
            "m_ref": (
                feats_ref[:, :min_len, :]
                if self.data_type == "angle"
                else feats_ref_rotmat[:, :min_len, :]
            ),
            "m_rst": (
                feats_rst[:, :min_len, :]
                if self.data_type == "angle"
                else feats_rst_rotmat[:, :min_len, :]
            ),
    ...
    def train_diffusion_forward(self, batch):
            ...
            if self.vae_type in ["mld", "vposert", "actor"]:
                ...
                if "interactee" in self.condition:
                    if self.predict_transl:
                        ...
                        z_cond, dist_cond = self.vae.encode(
                            f_ref_int, cond_image_emb, lengths
                        )
                    else:
                        # * Classifier free guidance
                        if self.do_classifier_free_guidance:
                            mask = (
                                torch.rand_like(feats_ref[:, :, 1])
                                < self.guidance_uncodp
                            )
                            feats_ref[:, :, 1] = torch.where(
                                mask, 0.0, feats_ref[:, :, 1]
                            )
                        z_cond, dist_cond = self.vae.encode(
                            feats_ref[:, :, 1], cond_image_emb, lengths
                        )
        ...
        if "scene" in self.condition and "image" not in self.condition:
            ...
            "interactee" in self.condition
            and "scene" not in self.condition
            and "image" not in self.condition
        ):
            cond_emb = z_cond
        else:
            cond_emb = None
    ...
    def ego_eval(self, batch):
        ...
        if "image" in self.condition and "scene" in self.condition:
            ...
            if self.pred_betas:
                ...
                if self.global_orient_egoego or self.transl_egoego:
                    ...
                    if self.pose_estimation_task:
                        ...
                        feats_ref, transl, beta, utils_, scene, length, dict_images = (
                            batch
                        )
            ...
            # * Classifier free guidance
            if self.do_classifier_free_guidance:
                empty_tensor_scene = torch.zeros(
                    (scene.shape[0], scene.shape[1], scene.shape[2])
                ).to(scene.device)
        ...
        if self.pred_betas:
            ...
            cam_feats = utils_[:, :, 0].unsqueeze(
                -1
            )  # bs,t,1  # + cam_feats  # [bs, 1]
            ...
            cam_feats = torch.cat(
                [bbox_info, cam_feats], dim=-1
            )  # [bbox_info] + cam_feats   # [bs, 3(+1)]

            # if self.with_cam_center:
            orig_fx = utils_[:, :, 0] * self.cfg.PROSCENE.CAM.FX_NORM_COEFF
            cam_center = torch.stack(
                [utils_[:, :, 1] / orig_fx, utils_[:, :, 2] / orig_fx], dim=-1
            )  # [bs, 2]
            cam_feats = torch.cat(
                [cam_center, cam_feats], dim=-1
            )  # [cam_center] + cam_feats   # [bs, 5(+1)]
            ...
            transl_feat = self.proscene2.encode_transl(
                transl_feat.reshape(-1, 3)
            ).reshape(transl_feat.shape[0], transl_feat.shape[1], -1)

            conditioning_feats_beta = torch.cat(
                [img_feats, scene_feats, transl_feat, cam_feats], dim=-1
            ).reshape(bs * t, -1)

            beta_predicted = self.proscene2.pred_betas(conditioning_feats_beta).reshape(
                bs, t, -1
            )
        ...
        if self.stage in ["diffusion", "vae_diffusion"]:
            ...
            if "interactee" in self.condition:
                if self.predict_transl:
                    ...
                    # * Classifier free guidance
                    if self.do_classifier_free_guidance:
                        empty_tensor_interactee = torch.zeros(
                            (f_ref_int.shape[0], f_ref_int.shape[1], f_ref_int.shape[2])
                        ).to(f_ref_int.device)
                        interactee_emb_uncond, _ = self.vae.encode(
                            empty_tensor_interactee, cond_image_emb, lengths
                        )
                        text_emb = torch.cat([interactee_emb_uncond, text_emb], dim=1)

                else:
                    text_emb, _ = self.vae.encode(
                        feats_ref[:, :, 1], cond_image_emb, lengths
                    )
        ...
        if self.data_type == "rot6d":
            ...
            if self.name_dataset == "egobody":
                ...
                # betas_rst = feats_rst[:, :min_len, 69:69+10].view(-1, 10)
                betas_rst = (
                    beta_predicted.reshape(-1, 10).float()
                    if self.pred_betas
                    else beta[:, idx_ref, :min_len, :].reshape(-1, 10).float()
                )
                # global_pose_rst = feats_rst[:, :min_len, :3].reshape(-1, 3).float()
                global_pose_rst = (
                    feats_ref[:, :min_len, :3].reshape(-1, 3).float()
                    if self.pred_global_orient == False
                    else feats_rst[:, :min_len, :3].reshape(-1, 3).float()
                )
                ...
                if "interactee" in self.condition:
                    ...
                    body_pose_int = (
                        feats_int[:, :min_len, 3:72].reshape(-1, 23 * 3).float()
                    )
                    ...
                    joint_int = self.smpl_model(
                        betas=betas_int,
                        body_pose=body_pose_int,
                        global_orient=global_pose_int,
                        pose2rot=True,
                        transl=transl_int,
                    )
                    ...
                    joints_int = joint_int.joints.reshape(
                        -1, feats_int.shape[1], 45, 3
                    )[:, :min_len, :24]
                ...
                if self.save_for_edo:
                    ...
                    head_pred = torch.cat(
                        [head_pred, orientation_quat_rst.reshape(-1, 4)], dim=-1
                    )
                    head_gt = torch.cat(
                        [head_gt, orientation_quat_ref.reshape(-1, 4)], dim=-1
                    )

                    head_pred = np.array(
                        self.get_root_matrix(head_pred.detach().cpu().numpy())
                    )
                    head_gt = np.array(
                        self.get_root_matrix(head_gt.detach().cpu().numpy())
                    )
                    ...
                    for btc in range(head_gt.shape[0]):
                        ...
                        head_orientation_error = self.get_frobenious_norm_rot_only(
                            head_gt_, head_pred_
                        )
                        print(frame_start)
                        if head_orientation_error < 0.3:
                            print(
                                "Saving, we have the following number of sequence: ",
                                len(self.dict_pred.keys()),
                            )

                            self.dict_pred[str(frame_start)] = (
                                vertices_rst[btc].detach().cpu().numpy()
                            )
                            self.dict_gt[str(frame_start)] = (
                                vertices_ref[btc].detach().cpu().numpy()
                            )
                            self.dict_int[str(frame_start)] = (
                                vertices_int[btc].detach().cpu().numpy()
                            )
                            # dict_pred_faces[name_dict] = faces_rst.detach().cpu().numpy()
                            # dict_gt_faces[name_dict] = faces_ref.detach().cpu().numpy()

                            np.save(
                                f"{to_save}/dict_pred_select_orientation_images.npy",
                                self.dict_pred,
                            )
                            np.save(
                                f"{to_save}/dict_gt_select_orientation_images.npy",
                                self.dict_gt,
                            )
                            np.save(
                                f"{to_save}/dict_int_select_orientation_images.npy",
                                self.dict_int,
                            )
                ...
                feats_rst = feats_rst.contiguous()
                feats_rst = torch.cat(
                    [feats_rst[:, :, :66], feats_rst[:, :, -3:]], dim=-1
                )  # ! Change back for training
                feats_rst = self.renorm(feats_rst)
                ...
                body_pose_rst = torch.cat(
                    [
                        body_pose_rst,
                        torch.zeros(body_pose_rst.shape[0], 6).to(
                            global_pose_rst.device
                        ),
                    ],
                    dim=-1,
                )
            ...
            if self.pose_estimation_task:
                ...
                body_pose_int_gt = (
                    f_int_gt[:, :min_len, 3:72].reshape(-1, 23 * 3).float()
                )
                ...
                joint_int_gt = self.smpl_model(
                    betas=betas_int_gt,
                    body_pose=body_pose_int_gt,
                    global_orient=global_pose_int_gt,
                    pose2rot=True,
                    transl=transl_int_gt,
                )
                joints_int_gt = joint_int_gt.joints.reshape(
                    -1, f_int_gt.shape[1], 45, 3
                )[:, :, :24]
        ...
        if self.name_dataset == "egobody":
            rs_set = {
                ...
                "joints_interactee_gt": (
                    joints_int_gt if self.pose_estimation_task else None
                ),
                "lengths": lengths,
                # "list_names": dict_images, # ! not declared, ignore it
            }
        elif self.name_dataset == "gimo":
            rs_set = {
                "m_ref": feats_ref,
    ...
    def t2m_eval(self, batch):
        ...
        if self.stage in ["diffusion", "vae_diffusion"]:
            ...
            text_emb = self.text_encoder(
                texts
            )  # * (128*2, 1, 768) it also embeds the uncond_tokens
            z = self._diffusion_reverse(text_emb, lengths)
        elif self.stage in ["vae"]:
            if self.vae_type in ["mld", "vposert", "actor"]:
                z, dist_m = self.vae.encode(motions, lengths)
